...
class UNET_AttentionBlock(nn.Module):
    ...
    def __init__(self, n_head: int, n_embed: int, d_context=config('CLIP_EMBEDDING_SIZE')):
        super().__init__()
...
class UNET(nn.Module):
    ...
    def __init__(self):
        ...
        self. encoders = self.get_encoder()
        self.bottleneck = self.get_bottleneck()
        self.decoders = self.get_decoder()
    ...
    def get_decoder(self):
        decoder = nn.Module([
            # (batch, 8*320 , height/64, width/64) -> (batch, 4*320 , height/64, width/64)
